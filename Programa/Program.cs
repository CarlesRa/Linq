...
namespace Programa
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\n\nA serializar se ha dicho");
            //Serializando
            string path = @"C:\Users\jcram\Documents\Interfaces graficas\interfaces graficas\equipos_data.xml";            
            XmlSerializer serializer = new XmlSerializer(typeof(List<Equipo>));
            
            using(TextWriter tw = new StreamWriter(path))
            {
                serializer.Serialize(tw, equipos);
            }
            ...
            List<Equipo> equiposIn = null;

            Console.WriteLine("\nDeserializando...");
            streamIn = File.OpenRead(path);
            XmlSerializer deserializer = new XmlSerializer(typeof(List<Equipo>));
            TextReader reader = new StreamReader(path);
            equiposIn = (List<Equipo>)deserializer.Deserialize(reader);
            equiposIn.ForEach(Console.WriteLine);

            Console.WriteLine("\n\nAHI TE QUEDAS DRAGON!!! El maestre Arturo nos dara su bendición");
            Console.ReadLine();
